...
namespace Leeflets\Core\Library\Filesystem;

use Leeflets\Core\Library\Config;
use Leeflets\Core\Library\Error;
use Leeflets\Core\Library\File;
 ...
 */
class FtpExt extends Filesystem {
	public $link;
	public $errors = null;
	public $options = array();

	function __construct(Config $config, $opt ) {
		$this->config = $config;
		$this->method = 'ftpext';
	...
	function delete( $file, $recursive = false, $type = false ) {
		...
		$filelist = $this->dirlist(File::trailingslashit( $file ) );
		if ( !empty( $filelist ) )
			foreach ( $filelist as $delete_file )
				$this->delete(File::trailingslashit( $file ) . $delete_file['name'], $recursive, $delete_file['type'] );
			return @ftp_rmdir( $this->link, $file );
	}
	...
	function is_dir( $path ) {
		$cwd = $this->cwd();
		$result = @ftp_chdir( $this->link, File::trailingslashit( $path ) );
	...
	function mkdir( $path, $chmod = false, $chown = false, $chgrp = false ) {
		$path = File::untrailingslashit( $path );
		if ( empty( $path ) )
			return false;
