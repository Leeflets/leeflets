...
class Curl {
	...
	function request( $url, $args = array() ) {
		...
		$theBody = '';
		$theHeaders = Http::processHeaders( $this->headers );
		...
		$response['message'] = Http::get_status_header_desc( $response['code'] );
		...
		if ( ! empty( $theHeaders['headers']['location'] ) && 0 !== $r['_redirection'] ) { // _redirection: The requested number of redirections
			if ( $r['redirection']-- > 0 ) {
				return $this->request(Http::make_absolute_url( $theHeaders['headers']['location'], $url ), $r );
			} else {
				return new Error( 'http_request_failed', __( 'Too many redirects.' ) );
			}
		}
     ...
     */
	public static function test(Hook $hook, $args = []) {
