...
class Streams {
	...
	function request( $url, $args = array() ) {
		...
		if ( ! $handle )
			return new Error( 'http_request_failed', sprintf( __( 'Could not open handle for fopen() to %s' ), $url ) );
		...
		if ( isset( $meta['wrapper_data']['headers'] ) )
			$processedHeaders = Http::processHeaders( $meta['wrapper_data']['headers'] );
		else
			$processedHeaders = Http::processHeaders( $meta['wrapper_data'] );
		...
		if ( ! empty( $strResponse ) && isset( $processedHeaders['headers']['transfer-encoding'] ) && 'chunked' == $processedHeaders['headers']['transfer-encoding'] )
			$strResponse = Http::chunkTransferDecode( $strResponse );
