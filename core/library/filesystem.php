...
class LF_Filesystem {
	...
	public $valid_methods = array( 'direct', 'ssh', 'ftpext', 'ftpsockets' );

	// Incomplete
	static function get_method( $config, $root_path ) {
		if ( in_array( $config->fs_method, $this->valid_methods ) ) {
			return $this->method = $config->fs_method;
		}

		if ( function_exists( 'getmyuid' ) && function_exists( 'fileowner' ) ) {
			$temp_file_name = $root_path . '/temp-write-test-' . time();
		...
		if ( isset( $args['connection_type'] ) && 'ssh' == $args['connection_type'] && extension_loaded( 'ssh2' ) && function_exists( 'stream_get_contents' ) ) $method = 'ssh2';
